...
const applicationsModulePath = join(rootDir, 'src/lib/api/applications/applications.ts');
const compatModulePath = join(rootDir, 'src/lib/api/applications/applications.compat.ts');

const compatModuleContent = `// This file is auto-generated. Do not edit manually.
...
const applicationsModuleContent = readFileSync(applicationsModulePath, 'utf8');
const hasGenericSectionHook = applicationsModuleContent.includes(
  'export const useApplicationsControllerUpsertSection',
);
const hasIdeaOverviewHook = applicationsModuleContent.includes(
  'export const useApplicationsControllerUpsertIdeaOverviewSection',
);

if (hasGenericSectionHook || !hasIdeaOverviewHook) {
  rmSync(compatModulePath, { force: true });
  console.log('✅ No API compatibility wrappers needed');
  process.exit(0);
}

mkdirSync(dirname(compatModulePath), { recursive: true });
writeFileSync(compatModulePath, `${compatModuleContent}\n`);

console.log(`✅ Generated compatibility wrapper at ${compatModulePath}`);
