...
export function OrganizationDashboardPlaceholder() {
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-4xl flex-col justify-center px-4 py-8">
      <section className="rounded-3xl border border-black/10 bg-white p-8 shadow-sm">
        ...
        <h1 className="mt-3 text-3xl font-semibold tracking-tight text-neutral-950">
          {t`Organization dashboard is the next slice`}
        </h1>
        <p className="mt-3 text-sm leading-6 text-neutral-600">
          {t`Your account is correctly routed away from the student dashboard. The organization dashboard implementation is still out of scope for this branch, so this entry point stays explicit instead of mixing company users into student UI.`}
        </p>
      </section>
    </main>
  );
...
export function SafeFallback({ role }: { role: string }) {
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-4xl flex-col justify-center px-4 py-8">
      <section className="rounded-3xl border border-black/10 bg-white p-8 shadow-sm">
        <h1 className="text-3xl font-semibold tracking-tight text-neutral-950">{t`Dashboard`}</h1>
        <p className="mt-3 text-sm leading-6 text-neutral-600">
          {t`The current dashboard branch is focused on student workflows. Your authenticated role is`}{' '}
          <span className="font-semibold text-neutral-900">{role}</span>,{' '}
          {t`so no student-only workspace is rendered here.`}
        </p>
      </section>
    </main>
  );
        ...
        <div className="rounded-[1.5rem] border border-[#dce5fb] bg-[#f8faff] p-5">
          ...
          <div className="mt-4">
            <Button asChild size="sm">
              <Link href={ROUTES.PROFILE}>{t`Review setup`}</Link>
            </Button>
          </div>
        </div>
        <div className="rounded-[1.5rem] border border-[#dce5fb] bg-[#f8faff] p-5">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <Button asChild size="sm" variant="outline">
              <Link href={ROUTES.PROGRAM_B_BACKLOG}>{t`Program B backlog`}</Link>
            </Button>
            <Button asChild size="sm" variant="outline">
              <Link href={ROUTES.TEAM}>{t`Team readiness`}</Link>
            </Button>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
            ...
            >
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {draftEntry ? (
                  <Button asChild size="sm">
                    <Link
                      href={ROUTES.studentApplication(draftEntry.applicationId)}
                    >{t`Continue draft`}</Link>
                  </Button>
                ) : (
                  <Button
                    size="sm"
        ...
        {activeCalls.length ? null : (
          <div className="rounded-[1.5rem] bg-[#f8faff] p-5">
            <p className="text-sm leading-7 text-[#58667d]">{t`No active Program A calls are available right now.`}</p>
          </div>
        )}
        {isLead ? (
              ...
              {draftEntries.length ? (
                draftEntries.map((entry) => (
                  ...
                  >
                    ...
                    <Button asChild size="sm" variant="outline">
                      <Link href={ROUTES.studentApplication(entry.applicationId)}>{t`Open`}</Link>
                    </Button>
                  </div>
                ))
              ) : (
      ...
      <div className="space-y-5">
        <div>
          <div className="mb-3 flex items-center justify-between gap-3">
            <h3 className="text-sm font-semibold tracking-[0.16em] text-[#6f7f9a] uppercase">{t`Opportunities`}</h3>
            <Button asChild size="sm" variant="outline">
              <Link href={ROUTES.PROGRAM_B_BACKLOG}>{t`Open backlog`}</Link>
            </Button>
          </div>
          <div className="space-y-3">
            {backlogPreview.map((item) => (
              ...
              >
                ...
                <div className="mt-3">
                  <Button asChild size="sm" variant="outline">
                    <Link href={ROUTES.programBBacklogDetail(item.id)}>{t`View details`}</Link>
                  </Button>
                </div>
              </div>
            ))}
            {backlogPreview.length ? null : (
              <p className="text-sm leading-7 text-[#58667d]">{t`Published Program B opportunities will appear here once available.`}</p>
            )}
          </div>
        </div>
        <div>
          <h3 className="mb-3 text-sm font-semibold tracking-[0.16em] text-[#6f7f9a] uppercase">{t`Active projects`}</h3>
          <div className="space-y-3">
            {projectPreview.map((project) => (
              ...
              >
                ...
                <div className="mt-3">
                  <Button asChild size="sm" variant="outline">
                    <Link href={ROUTES.programBProjectDetail(project.id)}>{t`Open project`}</Link>
                  </Button>
                </div>
              </div>
            ))}
